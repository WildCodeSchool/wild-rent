...
export const AccountDetails = () => {
  ...
  if (data) {
    const date = new Date(+data.getUserInfo.created_at);
              ...
              <section>
                ...
                <div className="mt-2 space-y-2">
                  <div>
                    ...
                    <div>{data.getUserInfo.first_name}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Nom</div>
                    <div>{data.getUserInfo.last_name}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Email</div>
                    <div>{data.getUserInfo.email}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Numéro de téléphone</div>
                    <div>{data.getUserInfo.phone_number}</div>
                  </div>
                  <div>
              ...
              <section>
                ...
                {data.getUserInfo.address ? (
                  <div className="mt-2 space-y-1">
                    <div>{data.getUserInfo.address.street}</div>
                    <div>
                      {data.getUserInfo.address.zipcode}{" "}
                      {data.getUserInfo.address.city}
                    </div>
                    <div>{data.getUserInfo.address.country}</div>
                  </div>
                ) : (
                  <AddressForm />
                )}
